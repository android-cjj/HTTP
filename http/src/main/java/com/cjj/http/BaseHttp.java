...
public class BaseHttp<T>{
    ...
    protected void baseDisplayLocalImage(T t,ImageView imageView,int resourceId,int placeholderId,int errorId)
    {
        if(t instanceof Activity) {
            Glide.with((Activity) t).load(resourceId).placeholder(placeholderId).error(errorId).into(imageView);
        }else if(t instanceof Fragment)
        {
            Glide.with((Fragment) t).load(resourceId).placeholder(placeholderId).error(errorId).into(imageView);
        }else if(t instanceof Context)
        {
            Glide.with((Context) t).load(resourceId).placeholder(placeholderId).error(errorId).into(imageView);
        }else
        {
            throw new RuntimeException("the t must be context or activity or fragment");
        }
    ...
    protected void baseDisplayImage(T t,ImageView imageView,String imgUrl,Drawable placeholderId,Drawable errorId)
    {
        if(t instanceof Activity) {
            Glide.with((Activity) t).load(imgUrl).placeholder(placeholderId).error(errorId).into(imageView);
        }else if(t instanceof Fragment)
    ...
    protected void baseDisplayLocalImage(T t,ImageView imageView,Drawable resourceId,Drawable placeholderId,Drawable errorId)
    {
        if(t instanceof Activity) {
            Glide.with((Activity) t).load(resourceId).placeholder(placeholderId).error(errorId).into(imageView);
        }else if(t instanceof Fragment)
